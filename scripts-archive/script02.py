...
if communication_edges_exist:
    print(
        "Communication network found. Will ignore data (if any) in /data/input/network/technical/"
    )
elif not (technical_edges_exist and technical_nodes_exist):
    print(
        "No network data found. Please provide network data in /data/input/communication/ and/or /data/input/technical/"
    )
else:
    ...
    nodes_communication, edges_communication, edges_communication_parallel = (
        technical_to_communication(node_gdf=nodes_studyarea, edge_gdf=edges_studyarea)
    )
    # save to files
    communication_folder = homepath + "/data/input/network/communication/"
    os.makedirs(communication_folder, exist_ok=True)
    nodes_communication.to_file(communication_folder + "nodes.gpkg", index=False)
    edges_communication.to_file(communication_folder + "edges.gpkg", index=False)
    edges_communication_parallel.to_file(
        communication_folder + "edges_parallel.gpkg", index=False
    )
