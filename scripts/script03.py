...
edgepath = homepath + "/data/input/network/communication/edges.gpkg"
...
for geomtype in ["point", "linestring", "polygon"]:
    ...
    if os.path.exists(geompath):
        ...
        for geomlayer in geomlayers:
            ...
            evaldict[geomtype][geomlayer_name]["bufferdistance"] = config_geomtype[geomlayer_name]
    ...
    for k, v in evaldict["point"].items():
        ...
        (
            ...
            res_current
                ...
                input_fp = v["filepath"],
                within_reach_output_fp = v["filepath"].replace("input", "output").replace(".gpkg", f"_within_{mydist}.gpkg"),
                outside_reach_output_fp = v["filepath"].replace("input", "output").replace(".gpkg", f"_outside_{mydist}.gpkg"),
                network_edges = edges,
                dist = mydist,
                name = k,
                type_col = "types",
...
if evaldict["polygon"]:
    for k, v in evaldict["polygon"].items():
        ...
        (
            ...
            input_fp = v["filepath"],
            output_fp = v["filepath"].replace("input", "output").replace(".gpkg", f"_{mydist}.gpkg"),
            network_edges = edges,
            dist = mydist,
            name = k,
            type_col = "types",
